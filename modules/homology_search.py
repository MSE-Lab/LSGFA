...
class DomainGroup:
    ...
    def homology_abc(self, input_file, blast_dir, threads, method, num):
        ...
        if method == 'diamond':
            ...
            blast_cmd = ' '.join([
                'diamond', 'blastp', '--more-sensitive', '-p', str(threads), '-q', input_file, '-d', self.db,
                '--evalue 1e-5 --outfmt tab qseqid sseqid pident', '--out', res, '-k', '0',
            ...
            blast_cmd = ' '.join([
                ...
                '-c 0.5'])
        # 两种方法的identity都统一为百分比值
        blast_cap = subprocess.Popen(blast_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        blast_cap.communicate()
        return res

    @staticmethod
    def filter_abc(input_file, res, ssn):
        """
        处理blast的结果文件
        """
        df = pd.read_csv(input_file, sep='\t', header=None, names=['query', 'subject', 'identity'])
        df['pair'] = df.apply(lambda row: tuple(sorted([row['query'], row['subject']])), axis=1)
        ...
        if not valid_pairs.empty:
            if ssn == '1':
                # RBH
                df[['sgenome']] = df[['subject']].map(lambda x: x.split('|')[0])
                filtered_data = df[df['query'] != df['subject']]
                df_sorted = filtered_data.sort_values(by=['identity'], ascending=[False])
                max_values = df_sorted.groupby(['query', 'sgenome']).agg({'identity': 'max'})  # .reset_index()
                ...
                # 固定阈值
                min_identity = 40  # 因为输出结果都统一为百分比的形式了
                filtered_df = df[df['identity'] >= min_identity]
                filtered_df[['query', 'subject', 'identity']].to_csv(res, sep='\t', index=False, header=False)
            elif ssn == '3':
                # 动态阈值
                min_identity = df[df['pair'].isin(valid_pairs)]['identity'].min()
                filtered_df = df[df['identity'] >= min_identity]
                filtered_df[['query', 'subject', 'identity']].to_csv(res, sep='\t', index=False, header=False)
        else:
            os.rename(input_file, res)
        return res
    ...
    def mcl_identity(self, input_abc, out_dir, threads, inflation=1.5):
        """
        使用blast的结果进行mcl的聚类
        使用的是identity，因为blast的结果就只保留了三列
        """
        res = os.path.join(out_dir, f'{self.name}.mcl')  # db的位置
        # 建db
        # message(text='Make database...', label='PROCESS')
        mcl_cmd = ' '.join(['mcl', input_abc, '--abc', '-I', str(inflation), '-o', res, '-te', str(threads)])
        mcl_cap = subprocess.Popen(mcl_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        mcl_cap.communicate()
        self.mcl = res

    def mcl_cc_file(self, out_dir):
        """
        使用blast的结果进行mcl的聚类
        使用的是identity，因为blast的结果就只保留了三列
        """
        with open(self.mcl, 'r') as f:
            lines = f.read().strip().split('\n')
        clucters = []
        for line in lines:
            cluster = line.split('\t')
            clucters.append(cluster)
        self.put_file(clucters, out_dir)  # 输出文件
